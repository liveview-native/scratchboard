defmodule ScratchboardWeb.Components.Modifiers.ViewConfiguration do
  ...
  def disabled_modifier(%{platform_id: :swiftui} = assigns) do
    ~SWIFTUI"""
    <VStack id="disabled">
      <Button modifiers={@native |> disabled(true)}>
        <Label>This button is disabled</Label>
      </Button>
      <Button modifiers={@native |> disabled(false)}>
  ...
  def hidden_modifier(%{platform_id: :swiftui} = assigns) do
    ~SWIFTUI"""
    <VStack id="hidden">
      <Text modifiers={@native |> hidden(@tick == :tick)}>
  ...
  def opacity_modifier(%{platform_id: :swiftui} = assigns) do
    ...
    <VStack id="opacity">
      ...
      <Text modifiers={opacity(opacity: 0.75)}>This text has 3/4 opacity</Text>
      <Text modifiers={opacity(opacity: 0.5)}>This text has 1/2 opacity</Text>
      <Text modifiers={opacity(opacity: 0.25)}>This text has 1/4 opacity</Text>
    </VStack>
    """
  end
  ...
  def preferred_color_scheme_modifier(%{platform_id: :swiftui} = assigns) do
    ~SWIFTUI"""
    <VStack id="preferred_color_scheme" modifiers={preferred_color_scheme(:light)}>
